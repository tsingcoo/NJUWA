...
#ifndef MODELH_COMMONS_H
#define MODELH_COMMONS_H
...
readfile_ch(string str, vector<vector<int>> &vv, unsigned int &cnt, unsigned int &max_line, string output_vcb_file) {
    ...
    word2int["null_wangql"] = j;//先增加一个null_wangql,主动加空值的一方其实是目标语言（噪声信道）
    ++j;
    ++j;//源语言序号第一个是0，然后从2开始
    ...
    cnt = j -1 ;//这里不需要减1是因为虽然序号从2算起，但是开头还有一个0抵消了,但是你忘了插入成功还要加1哦，所以要减1
    fin.close();
...
readfile_en(string str, vector<vector<int>> &vv, unsigned int &cnt, unsigned int &max_line, string output_vcb_file) {
    ...
    cnt = j - 2;//因为序号从2算起，所以个数要减1，但是你忘了插入成功还要加1哦，所以要减2
    fin.close();
...
void normalizeTable(unordered_map<int, unordered_map<int, double>> &t_count, vector<int> &nCh,
    ...
    if (it > 0) {
        normalizeTable(t_count, nCh, nEn, cnt_ch, cnt_en, it - 1);
    }
}
...
void HMMRealViterbi(unordered_map<int, int> &word_freq_ch, unordered_map<int, int> &word_freq_en, ofstream &fout,
                    ...
                    vector<double> &betainit_global, string align_op) {
    const int l = (int) v_ch.size() - 1;
    const int I = 2 * l;
    ...
    for (int j = 1; j <= J; ++j) {
        if (viterbi_alignment[j]) {
            ...
            if (align_op == "ch_en") {
                ...
                fout << j - 1 << "-" << viterbi_alignment[j] - 1;
            }
        }
    }
    fout << endl;
